...
import com.bpm.gd.wms.outbound.server.domain.BoxInfo;
import com.bpm.gd.wms.outbound.server.domain.BoxInfoProduct;
...
import com.bpm.gd.wms.outbound.server.mapper.base.OutBoundSaleShipmentMapper;
import com.bpm.gd.wms.outbound.server.mapper.ext.BaseDepotToMatterExtMapper;
import com.bpm.gd.wms.outbound.server.mapper.ext.BaseDepotToProductExtMapper;
import com.bpm.gd.wms.outbound.server.mapper.ext.CougnyTransferOrderExtMapper;
import com.bpm.gd.wms.outbound.server.mapper.ext.OutBoundFillBillExtMapper;
import com.bpm.gd.wms.outbound.server.mapper.ext.OutBoundFillBillTaskExtMapper;
import com.bpm.gd.wms.outbound.server.mapper.ext.OutBoundSaleExtMapper;
import com.bpm.gd.wms.outbound.server.mapper.ext.OutBoundSaleShipmentDetailsExtMapper;
import com.bpm.gd.wms.outbound.server.mapper.ext.OutBoundSaleShipmentExtMapper;
import com.bpm.gd.wms.outbound.server.mapper.ext.OutBoundWorkOrderExtMapper;
import com.bpm.gd.wms.outbound.server.mapper.ext.WarehousingProductReceiptExtMapper;
...
//        return ResultUtil.success(baseDepotToMatterExtMapper.updateCaseInfoSL(outBoundFillBillTask));
    ...
    @Override
    public ResultVO dockingAddInfo() {
        ...
        JSONObject jsonObject1 = JSONObject.parseObject(jsonObject.getString("payload"));
        JSONObject jsonObject2 = JSONObject.parseObject(jsonObject1.getString("std_data"));
        JSONObject jsonObject3 = JSONObject.parseObject(jsonObject2.getString("parameter"));
        JSONArray jsonArray = JSONArray.fromObject(jsonObject3.getString("ll"));
        List<OutBoundSaleShipmentDTO> list = JSONArray.toList(jsonArray, OutBoundSaleShipmentDTO.class);
        outBoundSaleShipmentExtMapper.insertList(list);
        return ResultUtil.success();
    }

    @Override
    public ResultVO dockingAddDetailsInfo() {
        RestTemplate restTemplate = new RestTemplate();
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON_UTF8);
        HttpEntity<String> entity = new HttpEntity<String>("", headers);
        ...
        JSONObject jsonObject3 = JSONObject.parseObject(jsonObject2.getString("parameter"));
        JSONArray jsonArray = JSONArray.fromObject(jsonObject3.getString("ll"));
        List<OutBoundSaleShipmentDetailsDTO> list = JSONArray.toList(jsonArray, OutBoundSaleShipmentDetailsDTO.class);
        outBoundSaleShipmentDetailsExtMapper.insertList(list);
        return ResultUtil.success();
    }

    @Override
    public ResultVO findShipment() {
         List<OutBoundSaleDTO> list =outBoundSaleShipmentExtMapper.findShipment();
         return ResultUtil.success(list);
    }

    @Override
    public ResultVO findShipmentByOrderCode(String shipmentOrderCode) {
        OutBoundSaleDTO outBoundSaleDTO = outBoundSaleShipmentExtMapper.findShipmentByOrderCode(shipmentOrderCode);
        return ResultUtil.success(outBoundSaleDTO);
    }
